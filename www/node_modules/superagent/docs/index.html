...
</code></pre><h2>Test documentation</h2><p>  The following <a href="docs/test.html">test documentation</a> was generated with <a href="http://visionmedia.github.com/mocha">Mocha&#39;s</a> &quot;doc&quot; reporter, and directly reflects the test suite. This provides an additional source of documentation.</p>
<h2>Request basics</h2><p> A request can be initiated by invoking the appropriate method on the <code>request</code> object, then calling <code>.end()</code> to send the request. For example a simple GET request:</p>
<pre><code> request
   .get(&#39;/search&#39;)
   .end(function(err, res){
...
</code></pre><h2>Setting header fields</h2><p>  Setting header fields is simple, invoke <code>.set()</code> with a field name and value:</p>
<pre><code> request
   .get(&#39;/search&#39;)
   .set(&#39;API-Key&#39;, &#39;foobar&#39;)
   .set(&#39;Accept&#39;, &#39;application/json&#39;)
...
<pre><code> request
   ...
   .end(callback);
</code></pre><h2>GET requests</h2><p> The <code>.query()</code> method accepts objects, which when used with the <strong>GET</strong> method will form a query-string. The following will produce the path <code>/search?query=Manny&amp;range=1..5&amp;order=desc</code>.</p>
<pre><code> request
   .get(&#39;/search&#39;)
   .query({ query: &#39;Manny&#39; })
   .query({ range: &#39;1..5&#39; })
...
</code></pre><h2>HEAD requests</h2><p>You can also use the <code>.query()</code> method for HEAD requests. The following will produce the path <code>/users?email=joe@smith.com</code>.</p>
<pre><code>  request
    .head(&#39;/users&#39;)
...
</code></pre><h2>POST / PUT requests</h2><p>  A typical JSON <strong>POST</strong> request might look a little like the following, where we set the Content-Type header field appropriately, and &quot;write&quot; some data, in this case just a JSON string.</p>
...
<h2>Setting the Content-Type</h2><p>  The obvious solution is to use the <code>.set()</code> method:</p>
<pre><code> request.post(&#39;/user&#39;)
   .set(&#39;Content-Type&#39;, &#39;application/json&#39;)
...
<pre><code> request.get(&#39;/user&#39;)
 ...
 request.post(&#39;/user&#39;)
   .accept(&#39;png&#39;)
</code></pre><h2>Query strings</h2><p>  <code>res.query(obj)</code> is a method which may be used to build up a query-string. For example populating <code>?format=json&amp;dest=/login</code> on a <strong>POST</strong>:</p>
<pre><code>request
  .post(&#39;/&#39;)
  .query({ format: &#39;json&#39; })
  .query({ dest: &#39;/login&#39; })
  ...
  .end(callback);
</code></pre><h2>Parsing response bodies</h2><p>  Super Agent will parse known response-body data for you, currently supporting <em>application/x-www-form-urlencoded</em>, <em>application/json</em>, and <em>multipart/form-data</em>.</p>
<h3>JSON / Urlencoded</h3><p>  The property <code>res.body</code> is the parsed object, for example if a request responded with the JSON string &#39;{&quot;user&quot;:{&quot;name&quot;:&quot;tobi&quot;}}&#39;, <code>res.body.user.name</code> would be &quot;tobi&quot;. Likewise the x-www-form-urlencoded value of &quot;user[name]=tobi&quot; would yield the same result.</p>
<h3>Multipart</h3><p>  The Node client supports <em>multipart/form-data</em> via the <a href="https://github.com/felixge/node-formidable">Formidable</a> module. When parsing multipart responses, the object <code>res.files</code> is also available to you. Suppose for example a request responds with the following multipart body:</p>
<pre><code>--whoop
...
<h2>Response properties</h2><p>  Many helpful flags and properties are set on the <code>Response</code> object, ranging from the response text, parsed response body, header fields, status flags and more.</p>
<h3>Response text</h3><p>  The <code>res.text</code> property contains the unparsed response body string. This
...
<h3>Response body</h3><p>  Much like SuperAgent can auto-serialize request data, it can also automatically parse it. When a parser is defined for the Content-Type, it is parsed, which by default includes &quot;application/json&quot; and &quot;application/x-www-form-urlencoded&quot;. The parsed object is then available via <code>res.body</code>.</p>
<h3>Response header fields</h3><p>  The <code>res.header</code> contains an object of parsed header fields, lowercasing field names much like node does. For example <code>res.header[&#39;content-length&#39;]</code>.</p>
<h3>Response Content-Type</h3><p>  The Content-Type response header is special-cased, providing <code>res.type</code>, which is void of the charset (if any). For example the Content-Type of &quot;text/html; charset=utf8&quot; will provide &quot;text/html&quot; as <code>res.type</code>, and the <code>res.charset</code> property would then contain &quot;utf8&quot;.</p>
<h3>Response status</h3><p>  The response status flags help determine if the request was a success, among other useful information, making SuperAgent ideal for interacting with RESTful web services. These flags are currently defined as:</p>
...
</code></pre><h2>Aborting requests</h2><p>  To abort requests simply invoke the <code>req.abort()</code> method.</p>
<h2>Request timeouts</h2><p>  A timeout can be applied by invoking <code>req.timeout(ms)</code>, after which an error
...
<h2>Basic authentication</h2><p>  Basic auth is currently provided by the <em>node</em> client in two forms, first via the URL as &quot;user:pass&quot;:</p>
...
<pre><code>request
  ...
  .end(callback);
</code></pre><h2>Following redirects</h2><p>  By default up to 5 redirects will be followed, however you may specify this with the <code>res.redirects(n)</code> method:</p>
<pre><code>request
  .get(&#39;/some.png&#39;)
  .redirects(2)
  .end(callback);
</code></pre><h2>Piping data</h2><p>  The Node client allows you to pipe data to and from the request. For example piping a file&#39;s contents as the request:</p>
...
req.pipe(stream);
</code></pre><h2>Multipart requests</h2><p>  Super Agent is also great for <em>building</em> multipart requests for which it provides methods <code>.attach()</code> and <code>.field()</code>.</p>
<h3>Attaching files</h3><p>  As mentioned a higher-level API is also provided, in the form of <code>.attach(name, [path], [filename])</code> and <code>.field(name, value)</code>. Attaching several files is simple, you can also provide a custom filename for the attachment, otherwise the basename of the attached file is used.</p>
<pre><code>request
  .post(&#39;/upload&#39;)
  ...
  .end(callback);
</code></pre><h3>Field values</h3><p>  Much like form fields in HTML, you can set field values with the <code>.field(name, value)</code> method. Suppose you want to upload a few images with your name and email, your request might look something like this:</p>
<pre><code> request
   .post(&#39;/upload&#39;)
   ...
   .end(callback);
</code></pre><h2>Compression</h2><p>  The node client supports compressed responses, best of all, you don&#39;t have to do anything! It just works.</p>
<h2>Buffering responses</h2><p>  To force buffering of response bodies as <code>res.text</code> you may invoke <code>req.buffer()</code>. To undo the default of buffering for text responses such
...
<h2>CORS</h2><p>  The <code>.withCredentials()</code> method enables the ability to send cookies
...
</code></pre><h2>Error handling</h2><p>Your callback function will always be passed two arguments: error and response. If no error occurred, the first argument will be null:</p>
<pre><code>request
 .post(&#39;/upload&#39;)
 .attach(&#39;image&#39;, &#39;path/to/tobi.png&#39;)
 .end(function(err, res){
...
</code></pre><h2>Generator support</h2><p>Superagent now supports easier control flow using generators. By using a generator control flow
...
<pre><code>var res = yield request
  .get(&#39;http://local&#39;)
  .auth(&#39;tobi&#39;, &#39;learnboost&#39;)
</code></pre><footer><div class="footer"> </div></footer></body></html>